-- ARCTor CRB1B hotfix
-- Fix confirmed purchase runtime failure caused by activity_events_source_check.
-- Existing activity source vocabulary allows system_event but not purchase_confirmation.
-- The purchase-confirmation provenance stays in metadata/source ids.
-- Baseline: main @ 428acd96081e2713a2261acaaba2a1e1b91befcc

begin;

update public.activity_templates
set
  default_source_type = 'system_event',
  default_metadata_json = coalesce(default_metadata_json, '{}'::jsonb)
    || jsonb_build_object(
      'eventSource', 'system_event',
      'evidenceWorkflow', 'purchase_confirmation'
    ),
...
comment on function public.materialize_confirmed_purchase_reality_v1(
  ...
  'CRB1B materializes one confirmed external purchase as one commerce transaction plus buyer Purchase and seller Sale actual activity_events. activity_events.source uses system_event; purchase_confirmation remains evidence/provenance in metadata.';
