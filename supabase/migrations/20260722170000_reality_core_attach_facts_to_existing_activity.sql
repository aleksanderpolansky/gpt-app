-- ARCTOR REALITY CORE R1-4E
-- ATTACH FACTS TO AN EXISTING ACTIVITY CONTAINER
-- Apply only after the R1-4B code package has passed production build.
--
-- Purpose:
--   Reuse an existing user-owned activity_events container when the
--   activity-review flow has already created it, instead of creating a
--   second normalized activity_event. The same RPC remains transactional.
--
...
--
-- The DDL is wrapped in one transaction. Any migration error rolls back
-- the index, function, grants and comment.
...
'ARCTor Reality Core transactional save RPC. Reuses an existing user-owned activity container when existingActivityEventId is supplied; otherwise creates a new event. Any failure rolls back all writes.';
