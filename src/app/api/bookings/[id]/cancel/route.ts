...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (
    booking.provider_actor_id !== personActor.id &&
    booking.receiver_actor_id !== personActor.id
  ) {
    return NextResponse.json(
      { error: "Only provider or receiver can cancel this booking" },
      { status: 403 }
    );
  }
  ...
  if (
    ...
    return NextResponse.json(
      { error: "Cancelled or completed bookings cannot be cancelled again" },
      { status: 400 }
    );
  }
