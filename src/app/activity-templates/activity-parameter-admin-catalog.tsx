...
const TECHNICAL_CODE_HELP: Readonly<Record<ActivityParameterLocale, string>> = {
  ...
  ru: "Стабильный системный идентификатор: строчные латинские буквы, цифры и знак подчёркивания; начинается с буквы. После создания изменить нельзя.",
  uk: "Стабільний системний ідентифікатор: малі латинські літери, цифри та знак підкреслення; починається з літери. Після створення змінити не можна.",
  de: "Stabile Systemkennung: lateinische Kleinbuchstaben, Ziffern und Unterstriche; beginnt mit einem Buchstaben. Nach der Erstellung nicht mehr änderbar.",
  es: "Identificador estable del sistema: letras latinas minúsculas, dígitos y guiones bajos; empieza por una letra. No puede cambiarse después de crear el parámetro.",
  cs: "Stálý systémový identifikátor: malá latinská písmena, číslice a podtržítka; začíná písmenem. Po vytvoření jej nelze změnit.",
};
  ...
  async function save() {
    ...
    try {
      const response = await fetch("/api/admin/activity-parameter-definitions", {
        ...
        body: JSON.stringify(editing ? { id: editing.id, parameterCode: editing.parameterCode, status: editing.status, ...semanticBody() } : { parameterCode: form.parameterCode, ...semanticBody() }),
      });
      const payload = await response.json();
      if (!response.ok || payload?.ok !== true) {
    ...
    <section className="rounded-[20px] border border-black/[0.07] bg-white p-4 shadow-sm sm:p-5">
            ...
            <article key={item.id} className="rounded-xl border border-slate-200 p-3">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className={`mt-1 inline-flex rounded-md border px-2 py-1 text-[10px] font-semibold ${item.semanticLocked ? "border-amber-200 bg-amber-50 text-amber-800" : "border-transparent bg-transparent text-slate-400"}`}>{item.semanticLocked ? copy.locked : copy.unlocked}</p>
                </div>
      ...
      {formVisible ? (
        <div ref={formRef} className="mt-5 scroll-mt-24 border-t border-slate-100 pt-4">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            {creating ? (
              <label className="block sm:col-span-2">
                <span className="text-xs font-medium">{copy.technicalCode}</span>
                <input
                  ...
                  spellCheck={false}
                  placeholder="count"
                  onChange={(event) => setForm((current) => ({ ...current, parameterCode: event.target.value.toLowerCase().replace(/\s+/gu, "_") }))}
                  className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 font-mono text-sm"
                />
                <p className="mt-1 text-[10px] leading-4 text-slate-500">{TECHNICAL_CODE_HELP[locale] ?? TECHNICAL_CODE_HELP.en}</p>
              </label>
            ) : (
              <div className="sm:col-span-2"><p className="text-xs font-medium">{copy.technicalCode}</p><p className="mt-1 rounded-xl bg-slate-50 px-3 py-2 font-mono text-xs text-slate-500">{editing?.parameterCode ?? ""}</p></div>
            )}
          ...
          <div className="mt-4 flex flex-wrap justify-end gap-2"><button type="button" onClick={closeForm} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold">{copy.cancel}</button><button type="button" disabled={busy || semanticLocked || (creating && !TECHNICAL_CODE_RE.test(form.parameterCode))} onClick={() => void save()} className="rounded-xl bg-[#3b6ef8] px-4 py-2 text-xs font-bold text-white disabled:opacity-50">{busy ? copy.saving : copy.save}</button></div>
        </div>
      ) : null}
    </section>
  );
