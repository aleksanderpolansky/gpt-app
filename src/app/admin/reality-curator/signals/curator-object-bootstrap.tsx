...
const COMBOBOX_COPY: Record<LocaleCode, { search: string; empty: string }> = {
  en: { search: "Search by name, description or English text…", empty: "No matching observation objects" },
  ru: { search: "Поиск по названию, описанию или английскому тексту…", empty: "Подходящие ОН не найдены" },
  pl: { search: "Szukaj po nazwie, opisie lub tekście angielskim…", empty: "Nie znaleziono pasujących obiektów obserwacji" },
  uk: { search: "Пошук за назвою, описом або англійським текстом…", empty: "Відповідних об’єктів спостереження не знайдено" },
  de: { search: "Nach Name, Beschreibung oder englischem Text suchen…", empty: "Keine passenden Beobachtungsobjekte gefunden" },
  es: { search: "Buscar por nombre, descripción o texto en inglés…", empty: "No se encontraron objetos de observación coincidentes" },
  cs: { search: "Hledat podle názvu, popisu nebo anglického textu…", empty: "Nebyly nalezeny odpovídající objekty pozorování" },
};
...
export function CuratorObjectBootstrap({
  ...
  const comboboxCopy = COMBOBOX_COPY[locale] ?? COMBOBOX_COPY.en;
    ...
    return (
      <div className="rounded-2xl border border-[#dce3f5] bg-[#f8faff] p-4">
        ...
        {decision === "existing_leaf_found" ? (
          <div className="mt-3 block text-xs font-bold text-[#4b5563]">
            <div>{copy.existingObject}</div>
            <ObservationObjectCombobox
              value={existingId}
              onChange={setExistingId}
              options={existingLeaves.map((item) => ({
                id: item.id,
                title: item.title,
              ...
              placeholder={copy.chooseExisting}
              searchPlaceholder={comboboxCopy.search}
              emptyLabel={comboboxCopy.empty}
              ariaLabel={copy.existingObject}
              disabled={busy}
            />
          </div>
        ) : null}
    ...
    <div className="rounded-2xl border border-[#dce3f5] bg-[#f8faff] p-4">
      ...
      {scope && nodeRole ? (
        <div className="mt-4 space-y-3">
          {nodeRole !== "root" ? (
            <div className="block text-xs font-bold text-[#4b5563]">
              <div>{copy.parent}</div>
              <ObservationObjectCombobox
                value={parentId}
                ...
                options={parents.map((item) => ({
                  ...
                  descriptionEn: item.descriptionEn,
                  canonicalKey: item.canonicalKey,
                  meta: `${roleLabel(item.nodeRole, copy)}${item.status === "draft" ? " · draft" : ""}`,
                }))}
                placeholder={copy.chooseParent}
                searchPlaceholder={comboboxCopy.search}
                emptyLabel={comboboxCopy.empty}
                ariaLabel={copy.parent}
                disabled={busy}
              />
            </div>
          ) : null}
