...
export default function DeletedOrganizationsPage() {
  ...
  const organizationsHref = `/organizations?locale=${locale}`;
  ...
  const loadOrganizations = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/organizations?scope=deleted", {
        method: "GET",
        cache: "no-store",
      });
  ...
  const hiddenCount = useMemo(() => organizations.length, [organizations]);
  ...
  return (
    <main className="min-h-full bg-[#f5f6fb] px-4 py-8 text-[#1a1d2e]">
      <div className="mx-auto grid w-full max-w-[1120px] gap-5">
        <header className="rounded-[18px] border border-[rgba(0,0,0,0.07)] bg-white p-6 shadow-[0_8px_24px_rgba(15,23,42,0.06)]">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                {t("organizations.actions.backToOrganizations")}
              </Link>
        ...
        <section className="rounded-[16px] border border-[rgba(0,0,0,0.07)] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
        ...
        {isLoading ? (
          <section className="rounded-[18px] border border-[rgba(0,0,0,0.07)] bg-white p-6 text-[14px] text-[#5a5f7a] shadow-[0_8px_24px_rgba(15,23,42,0.06)]">
            {t("organizations.deleted.loading")}
          </section>
        ) : null}

        {!isLoading && organizations.length === 0 ? (
          <section className="rounded-[18px] border border-[#dbeafe] bg-[#eff6ff] p-6 shadow-[0_8px_24px_rgba(15,23,42,0.06)]">
            <h2 className="text-[22px] font-bold text-[#1d4ed8]">
              {t("organizations.deleted.emptyHeading")}
            </h2>
            <p className="mt-2 text-[14px] leading-6 text-[#1d4ed8]">
              {t("organizations.deleted.emptyDescription")}
            </p>
          </section>
        ) : null}
              ...
              >
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2 text-[12px]">
                      ...
                      <span className="rounded-full border border-[#dfe3f1] bg-[#f8f9fd] px-3 py-1.5 font-semibold text-[#4a4f6a]">
                        ...
                        {formatOptional(organization.status, "hidden")}
                      </span>
                      <span className="rounded-full border border-[#dfe3f1] bg-[#f8f9fd] px-3 py-1.5 font-semibold text-[#4a4f6a]">
                        {t("organizations.card.catalog")}{" "}
                        {formatOptional(organization.directory_status, "hidden")}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid min-w-[240px] gap-2">
                    <Link
                      href={`/organizations/${organization.id}?locale=${locale}`}
                    ...
                    >
                      {t("organizations.deleted.openOrganization")}
                    </Link>
                  </div>
                </div>
              </article>
