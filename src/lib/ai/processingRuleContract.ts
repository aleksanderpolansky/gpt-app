...
export const SYSTEM_AI_GUARDS: readonly SystemGuardCatalogItem[] = [
  ...
  },
  {
    ...
    purpose: "Для русских и украинских сообщений короткая task-like формулировка, начинающаяся с инфинитива (например «выгулять собаку»), трактуется как намерение/указание и создаёт future/planned activity, если нет более сильного явного временного доказательства.",
    runtimeTargets: ["activity_quick_capture"],
    ...
    fullText: "RU/UK: первый смысловой токен после безопасных служебных префиксов проверяется как инфинитив. Очевидные существительные-исключения не считаются намерением. Прошедшая личная форма вроде «выгуливал» не становится future.",
    editable: false,
    precedenceRank: 300,
    changeMode: "code_release",
    whyLocked: "Это детерминированная грамматическая эвристика, меняющая temporalDirection и физическое место записи: календарь или журнал.",
