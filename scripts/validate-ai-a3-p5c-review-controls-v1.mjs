...
check(
  ...
  page.includes("operationId={analysisOperationId || null}") &&
    !page.includes("reviewActivityEventId && !reviewEditing\n                  ? null"),
);
check(
...
check(
  "REVIEW_MANUAL_PICKER_RECEIVES_ACTIVITY_ID",
  page.includes("activityEventId={reviewActivityEventId}"),
);
check(
  "MANUAL_PICKER_ACTIVITY_ID_PROP",
  page.includes("activityEventId = null,") &&
    page.includes("activityEventId?: string | null;"),
);
check(
...
check(
  ...
  page.includes("if (activityEventId) {") &&
    page.includes("activityEventId\n              ? \"Связь с ЦО сохранена") &&
    page.includes("будут материализованы как semantic_exposure после создания активности"),
);
check(
...
check("REVIEW_CONFIRM_BUTTON_PRESENT", page.includes('aria-label="Подтвердить"'));
check("REVIEW_REJECT_BUTTON_PRESENT", page.includes('aria-label="Отклонить"'));
check("REVIEW_PENCIL_PRESENT", page.includes('aria-label="Добавить объяснение"'));
check("REVIEW_WHY_PRESENT", page.includes('aria-label="Почему система так решила"'));
check("REVIEW_PLUS_PRESENT", page.includes("+ Добавить связь с ЦО"));
check(
  "REVIEW_NOT_AUTO_RESOLVED",
  !page.includes('quickCaptureReviewStatus: "resolved"') &&
    durable.includes('quickCaptureReviewStatus: "pending"'),
);
check(
  "DURABLE_HANDOFF_PRESERVED",
  durable.includes("raw_activity_signals") &&
    durable.includes("processing_status: \"pending\"") &&
    durable.includes('path: "/api/ai/reality/fact-materialize"'),
);
