...
const EVENT_ID = "13502589-b478-4c49-93c4-e793058e4b79";
...
function scanForTokens(value: unknown): {
  ...
  return {
    ...
    hasConfirmedCompleted:
      serialized.includes("confirmed_completed") ||
...
async function main() {
  ...
  const lifecycleResult =
    await ensureActivityEventRubricatorClassificationForKnownTemplate({
      ...
      processorName: "p479_r_a8d3_confirm_route_existing_event_diagnostic",
    });
  ...
  const checks = {
    ...
    eventLogsHaveConfirmedCompleted: eventLogTokens.hasConfirmedCompleted,
  ...
  const summary = {
    step: "P4.7.9-R-A8d-3",
    confirmRouteCompletedEvent: checks.eventStatus === "completed",
    routeEvidenceOk:
