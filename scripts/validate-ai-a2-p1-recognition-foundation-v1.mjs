...
function fail(msg) { console.error(JSON.stringify({validator:'AI_A2_P1',passed:false,error:msg},null,2)); process.exit(1); }
if (!fs.existsSync(migration)) fail('migration missing');
const buf = fs.readFileSync(migration);
const hash = crypto.createHash('sha256').update(buf).digest('hex');
if (hash !== expectedHash) fail('migration sha256 mismatch: ' + hash);
...
if (manifest.documentedState !== 'AI_A2_P1_RECOGNITION_FOUNDATION_V1') fail('manifest documentedState mismatch');
if (!manifest.gsr1lImplementation || manifest.gsr1lImplementation.liveAcceptance !== '14/14 PASS') fail('manifest AI-A2 acceptance missing');
if (manifest.gsr1lImplementation.runtimeIntegrated !== false) fail('runtimeIntegrated must remain false in P1');
if (!Array.isArray(manifest.manualAppliedSql) || !manifest.manualAppliedSql.includes(migrationRel)) fail('manualAppliedSql missing AI-A2 migration');
...
console.log(JSON.stringify({validator:'AI_A2_P1',passed:true,migrationSha256:hash,documentedState:manifest.documentedState,runtimeIntegrated:false},null,2));
