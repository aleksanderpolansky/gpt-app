...
check("data keeps independent scale config readable", contains(data, "scaleMode: input.config.scaleMode"));
...
check("builder persists independent scale for backward config compatibility", contains(builder, 'scaleMode: "independent"'));
check("builder keeps legacy single numeric config", contains(builder, "valueObjectId: selectedObservation.id"));

check(
  "V5 replacement keeps a visible custom legend for every fact series",
  contains(builder, "(data?.factSeries ?? []).map((series, index)") &&
    contains(builder, "multiSeriesDisplayName(series, locale)") &&
    contains(builder, "MULTI_SERIES_COLORS"),
);
check(
  "V5 replacement groups series into unit bands",
  contains(builder, "const multiSeriesBands = useMemo") &&
    contains(builder, "dashboardMeasurementUnitFamily(series.unit)"),
);
check(
  "V5 replacement removes per-series hidden Y axes",
  !contains(builder, 'yAxisId={`series-${index}`}') &&
    contains(builder, "ticks={band.ticks}"),
);
check(
  "V5 replacement keeps one bottom X axis",
  contains(builder, "hide={!isBottomBand}") &&
    contains(builder, "height={isBottomBand ? 28 : 0}"),
);
check(
  "V5 replacement synchronizes bands on one timeline",
  contains(builder, 'syncMethod="index"') &&
    contains(builder, 'syncId={"dashboard-unit-bands-" + block.id}'),
);
check("builder renders multiple lines", contains(builder, "band.series.map"));
check("builder renders presence as points", contains(builder, 'series.kind === "presence"') && contains(builder, "? 0"));
check("builder has six series colors", contains(builder, "MULTI_SERIES_COLORS"));
check("builder localizes V4/V5 copy", contains(builder, "MULTI_SERIES_COPY"));
...
check("V4 doc documents legacy mixed-unit independent scales", contains(v4Doc, "независим"));
