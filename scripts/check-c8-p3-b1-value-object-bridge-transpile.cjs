...
function main() {
  ...
  const resultPath = path.join(
    ...
    "docs",
    "value-objects",
    "category-derivation-bridge-c8-p3-b1-transpile-result.json",
  );
  ...
  const output = {
    ...
    note:
      "This is a targeted transpile/pattern smoke check for the additive additionalCategoryLinks type contract.",
  };

  fs.writeFileSync(resultPath, `${JSON.stringify(output, null, 2)}\n`, "utf8");

  console.log("P4.10.0-C8-P3-B1 — valueObjectBridge transpile smoke check");
  console.log("");
  ...
  console.log("");
  console.log("RESULT: PASS — additionalCategoryLinks type contract is present and transpiles.");
}
