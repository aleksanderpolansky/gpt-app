...
## 2026-09-12 — Общая связь ОН не должна блокироваться структурной ролью
Решение: в ручном «Конструкторе связей» ограничения `allowed_*_facet_codes` и `allowed_*_node_roles` не являются hard-block для curator action. Они сохраняются как advisory/audit compatibility snapshot. Hard guards остаются на identity/status системного ОН, закрытом реестре типа связи и запрете self-link.

## 2026-09-12 — Target ОН в Конструкторе последствий
Решение: целевой ОН выбирается только из листовых ОН, уже связанных с исходным ОН активной общей системной связью. Для сужения кандидатов направление общей связи не блокирует показ: incoming/outgoing обе стороны считаются доказательством связанности. Сам выбор target контекстный и записывается вместе с `activityTemplateId`, parameter/source pair и relation evidence. Одна исходная пара может иметь несколько target ОН. Формула остаётся отдельным следующим слоем.
...
## 2026-09-12 — Consequence candidate discovery не исключает глобальный набор source ОН
Решение: наличие ОН в качестве source какого-либо consequence-task не запрещает ему быть target другого task. Candidate discovery должен вычисляться относительно конкретной пары `current source ↔ other endpoint`, а не через глобальное вычитание всех `sourceIds`. Общая связь остаётся только фильтром допустимости; направление результирующего влияния задаётся на следующем слое формулы.

## 2026-09-12 — False-negative validator fan-out hotfix V1
Первая попытка V1 не дошла до ESLint/typecheck/build: production patch был материализован корректно, но validator сравнивал исходник с буквальной последовательностью `\\n` вместо реального перевода строки. V1.0.1 делает эти две проверки устойчивыми к форматированию через regex и не меняет production-логику.
