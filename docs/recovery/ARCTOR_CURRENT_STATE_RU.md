...
## ARCTOR_VO_AUTHORING_ANALYTICS_FOUNDATION_V1D
Дата: 2026-08-20
Baseline: 448c7f122a49e2b17ef7af3238b690a47849444b

V1A source release остановился до mutation на ошибке runner: литерал ${card.usageScope} был ошибочно интерполирован самим apply-hotfix.mjs. Source rollback PASS, Git baseline не изменился. V1B прошёл preflight, APPLY, validator 34/34 и A3.1 validator 125/125, но был остановлен fail-closed на ESLint: 9 no-unused-vars warnings; rollback PASS. V1C устранил warnings, но его validator остановился из-за собственной ошибки имени переменной: coefficientApi вместо объявленной oldCoefficientApi; rollback PASS, Git baseline не изменился. V1D исправляет именно эту validator scope-ссылку и продолжает тот же уже применённый и POSTCHECK=allPass:true DB-контракт.

Обычный private authoring закреплён за ontology root/intermediate/leaf; старый private draft-first изолирован. Commercial draft-first и отдельные product/service/gift-certificate/offer маршруты сохранены. Symptom создаётся как STATE/symptom_state leaf. Модель фиксации не меняется: activity/observation -> numeric facts -> leaf tags. Добавлен actor analytics overlay: единица накопленного эффекта, режим расчёта, baseline, target/critical boundary, направление, refresh period, inactivity delta, trend window, текущая тенденция и прогноз сроков. Параметр факта берётся прежде всего из metadata.parameterCode/systemParameterCode, затем из measure_type. Старый UI создания коэффициентов, изменяющих persisted fact value, удалён; его POST возвращает 410. SQL требует 0 активных старых правил и ставит DB trigger, запрещающий повторное создание active fact-mutating rules. Никакие существующие value_objects и коммерческие строки не удаляются.


## 2026-08-21 — ARCTOR_VO_BRANCH_DRIVEN_LEAF_AUTHORING_V1

- Для private observation authoring зафиксирована branch-driven модель: root / intermediate / leaf — только структурные позиции дерева; отдельного выбора вида листа нет.
- «Симптом» не является отдельным видом объекта наблюдения. Такие понятия, как боль, усталость или тошнота, создаются обычными leaf-объектами; их смысл задаётся веткой и полным structural path.
- Новый UI листа содержит только родительскую ветку, положение «лист», название и описание. `activity_pattern / symptom_state` больше не выбираются пользователем в этом authoring flow.
- Новый private root/intermediate/leaf после ontology create немедленно переводится существующим lifecycle RPC из `draft` в `active`. Commercial draft-first контур не меняется.
- Новые private intermediate nodes используют нейтральный совместимый ontology kind своей structural lineage: первый intermediate под DOMAIN root получает `ENTITY / generic_entity`, глубже technical facet наследуется только как storage compatibility. Этот facet не является пользовательским видом и не показывается как смысл private карточки.
- Новый private leaf наследует technical facet родительской intermediate-ветки и получает соответствующий generic kind. Это нужно только для совместимости с текущим P1C guard; классификация листа определяется его местом в дереве, а не picker-ом.
- Новые structural parent edges этого authoring flow записываются как `part_of`, а не `is_a`.
- Прямое создание leaf под root в новом flow запрещено: leaf создаётся под intermediate branch.
...
## 2026-08-28 — ARCTOR_TABLE_VIEWS_T2_1_SAFE_VALUE_OBJECT_EDITOR_V1_5

Baseline перед этапом: `main @ 3e664a4aa45c88f698c0b8c63cad2b8f8b0c6869`.

Table Views T1 закрыт как read-only этап. T2_1 добавляет opt-in безопасное редактирование title/description из Table View Observation Objects. `scope_code=global` и `origin_type_code=system` fail-closed read-only; ontology objects используют существующий `/ontology-definition` contract, drafts — существующий draft PATCH. Structural parent не редактируется напрямую и остаётся только в controlled restructure preview/apply flow. Activity Journal и Facts на этом этапе остаются read-only. SQL/schema migration в релизе отсутствуют. Предыдущие T2_1: V1 упал на Windows npm transport и откатился; V1_1 упал на рассинхронизации validator filename и откатился; V1_2 дошёл до release validator, где единственным FAIL был ложный recovery assertion, ожидавший буквальную строку `global/system` вместо проверки фактически записанных правил `scope_code=global` и `origin_type_code=system`; V1_2 также завершился ROLLBACK=PASS. V1_3 прошёл release validator, но changed-files ESLint остановил релиз на `react-hooks/set-state-in-effect` из-за синхронного setState внутри useEffect; V1_3 завершился ROLLBACK=PASS. V1_4 убрал этот effect и прошёл changed-files ESLint, но full TypeScript остановил релиз на TS18047: nullable prop valueObject использовался внутри async save closure после внешнего narrowing; V1_4 завершился ROLLBACK=PASS. V1_5 захватывает уже narrowed selection в immutable local const selectedValueObject и использует его во всех async write paths. Дополнительно V1_5 до mutation запускает отдельный semantic TypeScript preflight нового editor через project TypeScript с strict stubs, чтобы ошибки класса TS18047 ловились до изменения worktree. Package-path audit сохраняется. Будущий Documents/Spreadsheets/Mind Maps принцип: Guest/Local mode без обязательной регистрации для локального редактирования и Download; аккаунт — для cloud/save/history/collaboration/AI.

## 2026-08-28 — ARCTOR_TABLE_VIEWS_T2_2_INLINE_CELL_EDITING_V1

Baseline перед этапом: `main @ 3ce5f149818dac7cc0d30351d1f32746defbf5ee`.

T2_1 закрыт после production runtime-проверки: пользователь изменил описание Observation Object, сохранил, затем восстановил исходный текст; обе записи прошли через production UI. T2_2 переносит безопасное редактирование `title`/`description` непосредственно в ячейки Tabulator. Верхняя форма T2_1 больше не рендерится в Table View. Edit mode остаётся явным opt-in; редактирование запускается double-click только для разрешённых ячеек. `scope_code=global`, `origin_type_code=system` и объекты без поддерживаемого write-contract остаются fail-closed read-only. Ontology writes продолжают использовать `/ontology-definition` (`rename` / `semantic_definition` + idempotency key), drafts — существующий draft PATCH. При ошибке server write Tabulator восстанавливает предыдущее значение ячейки через `restoreOldValue()`; успешная запись обновляет React state и показывает явный `Saved` feedback. Structural parent, Role, Status, counters, Activity Journal и Facts не становятся редактируемыми. SQL/schema migration отсутствуют. Будущий Guest/Local Documents / Spreadsheets / Mind Maps принцип сохраняется без изменений.
