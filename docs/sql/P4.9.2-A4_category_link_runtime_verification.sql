/*
P4.9.2-A4
Targeted verification after value_object_category_links runtime integration.

Target event:
3e78af4d-dc23-454d-87d3-162f431a06e7
*/
...
WITH target_event AS (
  ...
  WHERE ae.id = '3e78af4d-dc23-454d-87d3-162f431a06e7'
),
...
old_voi_links AS (
  ...
  WHERE l.event_id = '3e78af4d-dc23-454d-87d3-162f431a06e7'
...
p492_category_links AS (
  SELECT
    ...
    cl.updated_at
  FROM public.value_object_category_links cl
  WHERE cl.metadata_json::text ILIKE '%runtime_category_link_from_bridge_mapping_metadata%'
     OR cl.metadata_json::text ILIKE '%p492%'
  ORDER BY cl.updated_at DESC NULLS LAST, cl.created_at DESC NULLS LAST
  LIMIT 30
),
...
usage_aggregates_for_target AS (
  ...
  WHERE ua.last_event_id = '3e78af4d-dc23-454d-87d3-162f431a06e7'
...
state_deltas AS (
  ...
  WHERE sd.event_id = '3e78af4d-dc23-454d-87d3-162f431a06e7'
...
daily_aggregates AS (
  ...
  WHERE da.last_event_id = '3e78af4d-dc23-454d-87d3-162f431a06e7'
...
state_snapshots AS (
  ...
  WHERE ss.last_event_id = '3e78af4d-dc23-454d-87d3-162f431a06e7'
...
counts AS (
  SELECT
    ...
    (SELECT count(*) FROM p492_category_links) AS p492_category_links_count,
    ...
    (SELECT count(*) FROM public.value_object_category_links) AS global_value_object_category_links_count,
    (SELECT count(*) FROM public.activity_event_value_object_links) AS global_new_v42_links_count,
...
SELECT
  '06_p492_category_links' AS section,
  COALESCE(jsonb_agg(to_jsonb(p492_category_links)), '[]'::jsonb) AS data
FROM p492_category_links

UNION ALL

SELECT
  '07_value_object_state_deltas' AS section,
...
SELECT
  '08_value_object_daily_aggregates' AS section,
...
SELECT
  '09_value_object_state_snapshots' AS section,
