/*
P4.9.4-A1
Object-cloud read/query layer audit.

Goal:
- read the current v4.2 object cloud without changing data;
...
latest_event_exposures AS (
  SELECT
    ...
    ROW_NUMBER() OVER (
      PARTITION BY l.value_object_id
...
daily_aggregate_summary AS (
  ...
  GROUP BY da.value_object_id
),

object_cloud_profile AS (
  SELECT
    occ.value_object_id,
    occ.category_table,
    occ.category_id,
  ...
  FROM object_category_cloud occ
  LEFT JOIN object_usage ou
    ON ou.value_object_id = occ.value_object_id
  LEFT JOIN latest_exposure_per_object le
    ON le.value_object_id = occ.value_object_id
  LEFT JOIN state_snapshot_summary sss
    ON sss.value_object_id = occ.value_object_id
  LEFT JOIN daily_aggregate_summary das
    ON das.value_object_id = occ.value_object_id
),
...
global_counts AS (
  SELECT
    (SELECT count(*) FROM public.activity_event_value_object_links) AS activity_event_value_object_links_count,
    (SELECT count(*) FROM public.value_object_usage_aggregates) AS value_object_usage_aggregates_count,
    (SELECT count(*) FROM public.value_object_category_links) AS value_object_category_links_count,
    (SELECT count(*) FROM public.value_object_state_snapshots) AS value_object_state_snapshots_count,
    (SELECT count(*) FROM public.value_object_daily_aggregates) AS value_object_daily_aggregates_count,
    (SELECT count(*) FROM object_cloud_profile) AS object_cloud_profile_rows_count,
...
SELECT
  '02_object_cloud_profile' AS section,
  COALESCE(jsonb_agg(to_jsonb(object_cloud_profile) ORDER BY contextual_category_slug, value_object_id), '[]'::jsonb) AS data
FROM object_cloud_profile

UNION ALL

SELECT
  '03_latest_event_exposures' AS section,
  COALESCE(jsonb_agg(to_jsonb(latest_event_exposures) ORDER BY exposure_created_at DESC NULLS LAST), '[]'::jsonb) AS data
FROM latest_event_exposures
